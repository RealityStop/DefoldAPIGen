...
public class JsonInheritanceConverter : JsonConverter
{
	...
	/// <summary>
	///     Initializes a new instance of the <see cref="JsonInheritanceConverter" /> class which only applies for the
	///     given base type.
	/// </summary>
		...
		JsonSerializer serializer)
	{
		...
		if (jObject.TryGetValue(_discriminator, out var tok))
		{
	...
	protected virtual Type GetDiscriminatorType(JObject jObject, Type objectType, string discriminatorValue)
	{
		...
		throw new InvalidOperationException("Could not find subtype of '" + objectType.Name +
		                                    "' with discriminator '" + discriminatorValue + "'.");
	}
